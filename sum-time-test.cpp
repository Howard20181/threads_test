...
void sumUpTasks(std::promise<unsigned long long>&& prom, const unsigned long long start, const unsigned long long end) {
	...
	do {
#ifdef _DEBUG
		if (unsigned_llong_overflow(sum, temp) == 1) return;
#endif // _DEBUG
		sum += temp;
		temp--;
	} while (temp >= start);
...
void sumUp(std::atomic<unsigned long long>& sum, const unsigned long long start, const unsigned long long end) {
	...
	do {
#ifdef _DEBUG
		if (unsigned_llong_overflow(tmpSum, temp) == 1)return;
#endif // _DEBUG
		tmpSum += temp;
		temp--;
	} while (temp >= start);
...
int main(int argc, char* argv[])
{
...
#ifdef _DEBUG
	std::cout << "DEBUG builds may cause performance degradation!" << std::endl << std::endl;
#endif // _DEBUG
	...
	std::cout << "Single-threaded:" << std::endl << "time=" << std::chrono::duration <double, std::milli>(diff).count() << "ms" << std::endl << "sum=" << sum << std::endl;
	// End Single-threaded
	...
	std::cout << "Multi-threaded:" << std::endl << "threads=" << thread_count << std::endl << "time=" << std::chrono::duration <double, std::milli>(diff).count() << "ms" << std::endl << std::setprecision(0) << "sum=" << sum;
	// End Multi-threaded
}
